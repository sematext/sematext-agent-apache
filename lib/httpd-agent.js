...
function ApacheHtttpdAgent (url, options) {
  ...
  if (options.emitMetricInterval && Number(options.sendInterval) > 0) {
    emitMetricInterval = Math.max((Number(options.sendInterval) || 10) * 1000, 20000)
  }
  statsInterval = (Number(options.collectionInterval) || 10) * 1000 || 10000
  if (statsInterval < 5000) {
    statsInterval = 5000
  }
  var ag = new Agent({
  ...
  if (options && options.phpFpmUrl && options.phpFpmUrl !== 'undefined') {
    ...
    ag.phpFpmMonitor = new PhpFpmMonitor({url: options.phpFpmUrl, collectionInterval: Number(options.collectionInterval) || 10, agent: ag, filterValue: filterValue})
  }
  return ag
}
