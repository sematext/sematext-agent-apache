...
function ApacheHtttpdAgent (url) {
  return new Agent({
    ...
    start: function (agent) {
      ...
      var portFilter = urlParser.parse(this.httpdStatusUrl).port || 80
      timerId = setInterval(function () {
        var aggMetrics = self.getAggregatedValues()
        var metrics = {type: 'httpd', filters: [portFilter], name: 'stats', fieldInfo: aggMetrics.keys, value: aggMetrics.values, sct: 'APP'}
